...
=begin
On line 62, we call the `Array#sort` method on an array of `Length` objects, which however
throws an ArgumentError because we did not define a `<=>` method for the `Length` class. Recall
that the `sort` method internally calls the `<=>` method of the object's class and uses the 
return value of the `<=>` to perform sorting. 

To fix this, we need to define a `<=>` method for the `Length` 
class. For the `<=>` method, we first convert the instance object and the `other`` object into a
  common unit and then compare their `values` via the `Integer#<=>` method. 
