...
=begin
The issue is when we want to call the `<<` method to add an Employee object, which raises
a NoMethodError for the getter method for the `@serial_number` instance variable. This error
is raised because the getter method is set to private, but the `==` method of the `Employee` 
class needs to access the value of `serial_number` between multiple class instances. This can be
  fixed by making the method `protected` rather than `private.`
